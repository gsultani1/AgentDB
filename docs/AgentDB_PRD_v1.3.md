...
**A Sovereign Agent Memory System with Tiered Knowledge, Empirical Knowledge Graph, Agent Communication Layer, Markdown Authoring, and External Chat Migration**
...
Expose an agent-facing API that any external AI system can call to retrieve contextual memories, match skills, check goals, and ingest new observations, enabling provider-agnostic memory augmentation.
...
Multi-user concurrent write access. AgentDB is designed for single-agent, single-operator use. Concurrency beyond SQLite WAL mode is out of scope.
...
The system is composed of four layers: the SQLite database layer containing all tables, triggers, views, and configuration; the Python sidecar backend providing all read/write operations, consolidation jobs, embedding generation, migration pipelines, markdown parsing, file watching, and both the operator API and agent API; the HTML/JS UI layer for human inspection, management, markdown authoring, and the chat console; and the Tauri shell providing native desktop packaging, sidecar process management, filesystem access, and OS integration.
...
| Agent API | JSON REST endpoints on localhost, called by external AI systems |
...
The agent communication layer is the interface through which external AI systems interact with AgentDB. It is distinct from the operator-facing management API. While the operator API serves the human UI, the agent API serves the AI system during inference, providing contextual memory retrieval and observation ingestion.

### 6.1 Agent API Endpoints

The agent API runs on the same Python backend as the operator API but under a separate /api/agent namespace. All endpoints accept and return JSON. Authentication is handled via a locally-stored API key configured in meta_config to prevent unauthorized local processes from accessing the agent's memory.

| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | /api/agent/context | Primary retrieval endpoint. Accepts a query string and optional filters (tier, entity, tags, time range, confidence threshold). Returns ranked memories across all tiers by embedding similarity, matched active goals, applicable skills, and relevant entity context. |
| POST | /api/agent/ingest | Stores a new observation in short_term_memory. Accepts content text, source identifier, optional entity hints, and optional session_id. Generates the embedding server-side. Returns the created memory ID. |
| POST | /api/agent/ingest/batch | Bulk ingestion for storing both user message and AI response from a single exchange. Accepts an array of observations. |
| POST | /api/agent/skill/match | Accepts a task description and returns ranked skills by semantic similarity, along with execution type, success rate, and dependency list. |
| POST | /api/agent/skill/execute | Triggers execution of a specific skill by ID against provided inputs. Logs execution result to update success_rate and use_count. |
| POST | /api/agent/goals/check | Accepts incoming context and returns any active goals whose embeddings match above the configured similarity threshold. |
| GET | /api/agent/identity | Returns all long_term_memory entries with category set to "identity" or "directive." Core persona and behavioral rules for every interaction. |
...
| GET | /api/agent/health | System status including database connectivity, sidecar uptime, embedding model availability, and last consolidation timestamp. |

### 6.2 Context Retrieval Pipeline

When the agent calls POST /api/agent/context, the backend executes a multi-stage retrieval pipeline. First, it generates an embedding from the query text. Second, it performs cosine similarity search across all three memory tiers, returning the top N results per tier (configurable in meta_config). Third, it identifies entities mentioned in the query and pulls their full context from the entities table plus all related memories via the relations table. Fourth, it checks the query embedding against active goal embeddings and returns any matches above the threshold. Fifth, it runs skill matching against the query to surface applicable capabilities. The complete context payload is returned as a single JSON response with clearly labeled sections for memories, entities, goals, and skills.

### 6.3 Provider-Agnostic Middleware
...
### 6.4 Context Snapshot on Every Exchange

Every conversation exchange that passes through the middleware automatically creates a context_snapshot row. This captures exactly which memories, skills, goals, and relations were used to inform that specific response. The snapshot is linked to the session and includes the outcome (the AI's response summary). This gives the operator complete forensic visibility into why the agent said what it said.
...
### 8.1 Short-to-Midterm Consolidation
...
### 8.2 Midterm-to-Long-Term Promotion
...
## 9. External Chat Migration Pipeline
...
### 9.5 Phase 4: Graph Construction
...
Landing page showing aggregate statistics: total memories per tier, total entities, active goals, unresolved contradictions, pending feedback items, last consolidation timestamp, database file size, embedding model info, configured LLM provider, sidecar health status, and file watcher status (enabled/disabled, inbox path, files pending).

### 10.2 Memory Browser

Tabbed interface for short-term, midterm, and long-term memory tables. Each tab provides sortable columns, text search, semantic search (enter a query, backend returns ranked results by embedding similarity), and filters for confidence, decay weight, category, date range, entity, tags, and provenance source. Clicking a memory row expands it to show full content, provenance chain, linked entities, related memories via the relations table, and any feedback history. An "Edit as Markdown" button opens the record in the markdown editor for modification.

### 10.3 Mind Map / Knowledge Graph
...
### 10.7 Feedback and Contradictions
...
Editor for all meta_config values including memory behavior settings (decay rates, promotion thresholds, TTL defaults, consolidation interval), LLM provider configuration (provider, API key, model, endpoint), and markdown authoring settings (inbox path, file watcher enabled/disabled, polling interval). API key fields are masked by default with a reveal toggle. Includes a database maintenance section with buttons for manual consolidation trigger, integrity check, embedding cache rebuild, and database export/backup. In Tauri mode, includes database file path management and sidecar process controls.

---

## 11. API Endpoints

The Python backend exposes two API namespaces on localhost. The operator API (/api/) serves the management UI. The agent API (/api/agent/) serves external AI systems and the chat console. All responses follow a consistent {status, data, error} envelope.
...
| GET | /api/memories/{tier} | List memories with pagination, filters, and sort |
| GET | /api/memories/{tier}/{id} | Single memory with full relations and provenance |
| POST | /api/memories/search | Semantic search across all tiers |
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | /api/agent/context | Retrieve ranked memories, goals, skills, and entity context for a query |
| POST | /api/agent/ingest | Store a single observation in short_term_memory |
| POST | /api/agent/ingest/batch | Bulk ingest multiple observations (e.g., user message + AI response) |
| POST | /api/agent/skill/match | Find applicable skills by semantic similarity to a task description |
...
| GET | /api/agent/identity | Retrieve all long-term identity and directive memories for persona injection |
...
| GET | /api/agent/health | System status for sidecar health monitoring |
| POST | /api/agent/chat | Full pipeline endpoint: accepts user message, retrieves context, calls LLM provider via middleware, ingests exchange, returns response with full observability payload |
...
SQLite schema creation with all twenty-one tables including skill_implementations. Trigger implementation for polymorphic referential integrity and cascade deletes. meta_config seeded with defaults. Python data access module with CRUD operations for all tables. Embedding generation pipeline using sentence-transformers. Basic CLI for database initialization and manual operations.

### Phase 2: Agent Communication Layer

Agent API endpoints (context retrieval, ingestion, skill matching, goal checking, identity, session management). Context retrieval pipeline with multi-tier semantic search. Provider-agnostic middleware module with adapter interface. Claude and OpenAI adapters. Local LLM adapter. Context snapshot generation on every exchange.
...
Python HTTP server with both API namespaces. Single-file HTML/CSS/JS application. Dashboard, memory browser with "Edit as Markdown" integration, mind map visualization, chat console with observability sidebar, markdown editor with type-aware frontmatter templates and batch import, skills manager with implementation version history, feedback/contradiction resolution views, import manager, audit log viewer, and settings editor.

### Phase 7: Tauri Packaging

Tauri project setup with Python sidecar configuration. Sidecar lifecycle management (spawn, health monitoring, restart, graceful shutdown). Native file dialogs for imports, markdown files, and database management. System tray integration for background operation. Build pipeline producing distributable executables for Windows, macOS, and Linux.
...
Embeddings cache implementation. SQLite WAL mode tuning. Query performance profiling and index optimization. Periodic integrity check job. Database backup and export tooling. API key encryption at rest. Documentation.

---

## 13. Success Criteria

The database initializes from a single Python command and produces a valid, self-contained .db file with all tables, triggers, indexes, and default configuration.

The agent API context endpoint returns a ranked, multi-tier context payload in under 500ms for a database containing 10,000+ memories.
...
The chat console displays the full observability payload (retrieved memories, matched goals, applicable skills, ingested observations) for every exchange in real time.
...
The entire system runs offline with zero external API calls after initial embedding model download (LLM provider calls excluded when using cloud-hosted models).
