  ...
  V.load = function() {
    el().innerHTML = `
      ...
      <div class="search-bar">
        ...
        <button class="btn" onclick="swadb.views.audit.loadList()">Filter</button>
      </div>
  ...
  V.loadList = async function() {
    ...
    c.innerHTML = '<table><thead><tr><th>Timestamp</th><th>Table</th><th>Operation</th><th>Row ID</th><th>Triggered By</th></tr></thead><tbody>' +
      r.data.map(function(a) {
        return '<tr><td style="font-size:12px;white-space:nowrap">' + swadb.esc((a.timestamp||'').substring(0,19)) +
          '</td><td>' + swadb.esc(a.table_name) +
          '</td><td>' + swadb.esc(a.operation) +
          '</td><td style="font-family:var(--mono);font-size:12px">' + swadb.esc((a.row_id||'').substring(0,8)) +
          '</td><td>' + swadb.esc(a.triggered_by) + '</td></tr>';
      }).join('') + '</tbody></table>';
  };
})();
